...
export const fields = ['cacheSize', 'blockSize', 'associativity', 'fetchAddress']
...
const validate = values => {
  ...
  if (!values.cacheSize) {
    ...
    } else if (isNaN(Number(values.fetchAddress))) {
      errors.fetchAddress = 'Must be a positive integer'
    } else if (!Number.isInteger(Number(values.fetchAddress))) {
      errors.fetchAddress = 'Must be a positive integer'
    } else if (Number(values.fetchAddress) < 0) {
      errors.fetchAddress = "Must be a positive integer"
    }
    return errors
  }
  ...
  class CacheForm extends React.Component {
    render() {
      const { fields: { cacheSize, blockSize, associativity, fetchAddress }, resetForm, handleSubmit, submitting } = this.props
      return (
